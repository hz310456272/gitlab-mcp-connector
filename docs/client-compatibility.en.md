...
| Cursor | Tested | MCP server loads with 16 tools enabled; single-tool `gitlab_list_branches` call returns expected branches |
...
- **Read-only**: all 16 tools are read-only. Verification flows below intentionally use only read operations.
...
After editing, reload the Cursor MCP settings. The MCP panel should show `gitlab` with **16 tools enabled**.
