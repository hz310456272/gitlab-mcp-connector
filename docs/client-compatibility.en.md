...
| Cursor | Tested | MCP server loads with 27 tools enabled; single-tool `gitlab_list_branches` call returns expected branches |
...
- **Read-only**: all 27 tools are read-only. Verification flows below intentionally use only read operations.
...
After editing, reload the Cursor MCP settings. The MCP panel should show `gitlab` with **27 tools enabled**.
