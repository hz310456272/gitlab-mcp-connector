...
| Cursor | 已验证 | MCP 面板显示 26 tools enabled，单工具 `gitlab_list_branches` 调用返回预期分支 |
...
改完后在 Cursor 里 reload MCP 设置。MCP 面板应能看到 `gitlab`，并显示 **26 tools enabled**。
