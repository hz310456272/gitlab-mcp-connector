...
**默认行为不变**：当前 29 个只读工具全部暴露，无需任何 toolset 配置。
...
| `read.ci` | `gitlab_get_pipeline_jobs`、`gitlab_get_job_log`、`gitlab_get_ci_config`、`gitlab_list_job_artifacts`、`gitlab_get_job_artifact_file` | 是 |
...
> - 即使配置了这些字段，也不会改变 29 个只读工具全部暴露的行为
>
